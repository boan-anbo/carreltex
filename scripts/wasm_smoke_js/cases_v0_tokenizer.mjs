export function runTokenizerCases(ctx, helpers) {
  ...
  const textdegreeMainBytes = new TextEncoder().encode('\\documentclass{article}\n\\begin{document}\nHello.\\textdegree XYZ\n\\end{document}\n');
  if (addMountedFile('main.tex', textdegreeMainBytes, 'tokenizer_control_word_textdegree_main') !== 0) {
    throw new Error('mount_add_file(tokenizer control-word-textdegree main.tex) failed');
  }
  if (ctx.mountFinalize() !== 0) {
  ...
  expectOk(ctx.compileMain(), 'compile_main_v0(tokenizer control-word-textdegree)');
  {
    ...
    if (helloBaselineCharCount === null) {
      throw new Error('helloBaselineCharCount not initialized for tokenizer control-word-textdegree case');
    }
    if (stats.char_count !== helloBaselineCharCount + 4) {
  ...
  const textdaggerMainBytes = new TextEncoder().encode('\\documentclass{article}\n\\begin{document}\nHello.\\textdagger XYZ\n\\end{document}\n');
  if (addMountedFile('main.tex', textdaggerMainBytes, 'tokenizer_control_word_textdagger_main') !== 0) {
    throw new Error('mount_add_file(tokenizer control-word-textdagger main.tex) failed');
  }
  if (ctx.mountFinalize() !== 0) {
  ...
  expectOk(ctx.compileMain(), 'compile_main_v0(tokenizer control-word-textdagger)');
  {
    ...
    if (helloBaselineCharCount === null) {
      throw new Error('helloBaselineCharCount not initialized for tokenizer control-word-textdagger case');
    }
    if (stats.char_count !== helloBaselineCharCount + 4) {
