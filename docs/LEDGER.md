...
| `crates/carreltex-engine/src/lib.rs` | engine | compile-seam-v0 | verified | `cargo test --manifest-path crates/carreltex-engine/Cargo.toml` | Compile behavior seam; tokenizer is now a hard validation gate, deterministic bounded compile logs, and explicit `main.xdv` artifact seam (empty until engine wired) |
