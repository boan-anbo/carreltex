...
fn pdf_renderer_center_alignment_keeps_wrapped_continuation_centered_v1() {
    ...
    let expected_start_width_pt =
        segment_width_pt_v0(b"CENTERSTART alpha ") + scaled_segment_width_pt_v0(b"mid", 64);
    ...
    assert!(
        centered_line.contains("64 Tz") && centered_line.contains("(mid) Tj 100 Tz"),
...
fn pdf_renderer_right_alignment_keeps_wrapped_continuation_right_v1() {
    ...
    let layout = parse_dvi_v2_text_page_to_layout_v0(&xdv, 786_432).expect("layout parse");
    let expected_start_width_pt =
        segment_width_pt_v0(b"RIGHTSTART edge, ") + scaled_segment_width_pt_v0(b"core", 64);
    ...
    assert!(
        right_line.contains("64 Tz") && right_line.contains("(core) Tj 100 Tz"),
