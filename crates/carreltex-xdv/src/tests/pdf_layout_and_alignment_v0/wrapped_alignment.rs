...
#[test]
fn pdf_renderer_wrapped_aligned_plain_bundle_short_and_medium_gaps_are_tightened_v745() {
    let cases = [
        (
    ...
    for (input, needle, max_gap_pt, label) in cases {
        ...
        assert!(
            ...
            "{label} should stay tightened in the v745 bundle: actual_gap={actual_gap}, max_gap_pt={max_gap_pt}"
        );
    }
}

#[test]
fn pdf_renderer_wrapped_aligned_plain_center_right_medium_continuity_stays_coherent_v745() {
    ...
    assert!(
        center_gap <= 8.91 && right_gap <= 8.91,
...
#[test]
fn pdf_renderer_wrapped_aligned_plain_acceptance_surface_stays_coherent_v745() {
    let xdv = write_dvi_v2_text_page_with_layout_and_wrap_v0(
    ...
    assert!(
        short_center_gap <= 8.91
            && short_right_gap <= 8.91
