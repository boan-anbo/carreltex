...
#[test]
fn pdf_renderer_wrapped_aligned_plain_bundle_short_and_medium_gaps_are_tightened_v801() {
    let cases = [
        (
    ...
    for (input, needle, max_gap_pt, label) in cases {
        ...
        assert!(
            ...
            "{label} should stay tightened in the v801 bundle: actual_gap={actual_gap}, max_gap_pt={max_gap_pt}"
        );
    }
}

#[test]
fn pdf_renderer_wrapped_aligned_plain_center_right_medium_continuity_stays_coherent_v801() {
    ...
    assert!(
        center_gap <= 6.11 && right_gap <= 6.11,
...
#[test]
fn pdf_renderer_wrapped_aligned_plain_acceptance_surface_stays_coherent_v801() {
    let xdv = write_dvi_v2_text_page_with_layout_and_wrap_v0(
    ...
    assert!(
        short_center_gap <= 6.11
            && short_right_gap <= 6.11
