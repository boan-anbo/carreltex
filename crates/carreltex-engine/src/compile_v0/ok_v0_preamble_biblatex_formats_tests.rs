...
#[test]
fn preamble_biblatex_formats_bundle_is_ok_and_output_matches_without_decls() {
    let with_decls = compile_main(
        b"\\documentclass{article}\\DeclareFieldFormat*{title}{#1}\\DeclareNameFormat{author}{#1}\\DeclareDelimFormat{multinamedelim}{\\addcomma\\space}\\DeclareDelimAlias{finalnamedelim}{multinamedelim}\\DeclareBibliographyDriver{book}{#1}\\DefineBibliographyStrings{english}{andothers={et\\addabbrvspace al\\adddot}}\\begin{document}HelloWorld\\end{document}",
    );
    let without_decls =
...
#[test]
fn preamble_biblatex_formats_missing_defs_group_is_not_implemented() {
    let result = compile_main(
        b"\\documentclass{article}\\DeclareNameFormat{author}\\begin{document}HelloWorld\\end{document}",
    );
...
#[test]
fn preamble_biblatex_formats_begin_in_defs_is_not_implemented() {
    let result = compile_main(
        b"\\documentclass{article}\\DeclareBibliographyDriver{book}{\\begin{document}}\\begin{document}HelloWorld\\end{document}",
    );
    assert_eq!(result.status, CompileStatus::NotImplemented);
    assert!(result.main_xdv_bytes.is_empty());
}
