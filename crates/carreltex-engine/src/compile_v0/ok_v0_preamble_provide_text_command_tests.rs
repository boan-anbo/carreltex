...
#[test]
fn provide_text_command_preamble_is_ok_and_output_matches_without_it() {
    let with_decl = compile_main(
        b"\\documentclass{article}\\ProvideTextCommand{\\Foo}{T1}{ABC}\\begin{document}HelloWorld\\end{document}",
    );
    let without_decl =
...
#[test]
fn provide_text_command_bad_first_arg_shape_is_not_implemented() {
    let result = compile_main(
        b"\\documentclass{article}\\ProvideTextCommand{Foo}{T1}{ABC}\\begin{document}HelloWorld\\end{document}",
    );
    assert_eq!(result.status, CompileStatus::NotImplemented);
