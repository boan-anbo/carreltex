...
#[test]
fn preamble_fontspec_config_bundle_is_ok_and_output_matches_without_decls() {
    let with_decls = compile_main(
        b"\\documentclass{article}\\setmainfont[Numbers=OldStyle]{Latin Modern Roman}\\setsansfont{Latin Modern Sans}\\setmonofont[Scale=0.9]{Latin Modern Mono}\\sisetup{round-mode=places,round-precision=2}\\lstset{basicstyle=\\ttfamily,breaklines=true}\\microtypesetup{activate=true}\\begin{document}HelloWorld\\end{document}",
    );
    let without_decls =
...
#[test]
fn preamble_setmainfont_missing_group_is_not_implemented() {
    let result = compile_main(
        b"\\documentclass{article}\\setmainfont[Scale=1.0]\\begin{document}HelloWorld\\end{document}",
    );
...
#[test]
fn preamble_sisetup_begin_in_group_is_not_implemented() {
    let result = compile_main(
        b"\\documentclass{article}\\sisetup{\\begin{document}}\\begin{document}HelloWorld\\end{document}",
    );
    assert_eq!(result.status, CompileStatus::NotImplemented);
    assert!(result.main_xdv_bytes.is_empty());
}
