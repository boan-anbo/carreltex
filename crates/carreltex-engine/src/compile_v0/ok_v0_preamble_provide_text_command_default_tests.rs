...
#[test]
fn provide_text_command_default_preamble_is_ok_and_output_matches_without_it() {
    let with_decl = compile_main(
        b"\\documentclass{article}\\ProvideTextCommandDefault{\\Foo}{ABC}\\begin{document}HelloWorld\\end{document}",
    );
    let without_decl =
...
#[test]
fn provide_text_command_default_body_with_controlseq_is_not_implemented() {
    let result = compile_main(
        b"\\documentclass{article}\\ProvideTextCommandDefault{\\Foo}{\\cite{X}}\\begin{document}HelloWorld\\end{document}",
    );
...
#[test]
fn provide_text_command_default_bad_first_arg_shape_is_not_implemented() {
    let result = compile_main(
        b"\\documentclass{article}\\ProvideTextCommandDefault{Foo}{ABC}\\begin{document}HelloWorld\\end{document}",
    );
    assert_eq!(result.status, CompileStatus::NotImplemented);
