...
#[test]
fn preamble_length_counter_bundle_is_ok_and_output_matches_without_commands() {
    let with_commands = compile_main(
        b"\\documentclass{article}\\setcounter{tocdepth}{2}\\addtolength{parindent}{1em}\\setlength{parskip}{0pt}\\begin{document}HelloWorld\\end{document}",
    );
...
#[test]
fn preamble_length_counter_missing_group_is_not_implemented() {
    let result = compile_main(
        b"\\documentclass{article}\\setcounter{tocdepth}\\begin{document}HelloWorld\\end{document}",
    );
