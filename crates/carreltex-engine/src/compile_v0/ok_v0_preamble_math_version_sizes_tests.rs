...
#[test]
fn math_version_sizes_preamble_decls_are_ok_and_output_matches_without_them() {
    let with_decls = compile_main(
        b"\\documentclass{article}\\DeclareMathVersion{bold}\\mathversion{normal}\\DeclareMathSizes{10}{8}{6}{5}\\begin{document}HelloWorld\\end{document}",
    );
    let without_decls =
...
#[test]
fn declare_math_sizes_missing_group_is_not_implemented() {
    let result = compile_main(
        b"\\documentclass{article}\\DeclareMathSizes{10}{8}{6}\\begin{document}HelloWorld\\end{document}",
    );
...
#[test]
fn mathversion_group_with_control_seq_is_not_implemented() {
    let result = compile_main(
        b"\\documentclass{article}\\mathversion{\\foo}\\begin{document}HelloWorld\\end{document}",
    );
    assert_eq!(result.status, CompileStatus::NotImplemented);
    assert!(result.main_xdv_bytes.is_empty());
}
