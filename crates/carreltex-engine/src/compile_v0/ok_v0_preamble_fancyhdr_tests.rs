...
#[test]
fn preamble_fancyhf_bundle_is_ok_and_output_matches_without_commands() {
    let with_commands = compile_main(
        b"\\documentclass{article}\\usepackage{fancyhdr}\\pagestyle{fancy}\\fancyhf{\\thepage}\\begin{document}HelloWorld\\end{document}",
    );
...
#[test]
fn preamble_fancyhead_and_fancyfoot_optional_brackets_are_ok() {
    let with_commands = compile_main(
        b"\\documentclass{article}\\fancyhead[LE,RO]{\\leftmark}\\fancyfoot[CE]{\\thepage}\\begin{document}HelloWorld\\end{document}",
    );
    let without_commands =
        compile_main(b"\\documentclass{article}\\begin{document}HelloWorld\\end{document}");
    assert_eq!(with_commands.status, CompileStatus::Ok);
...
#[test]
fn preamble_fancypagestyle_nested_defs_are_ok() {
    let with_commands = compile_main(
        b"\\documentclass{article}\\fancypagestyle{plain}{\\fancyhf{}\\fancyhead[RO]{X}}\\begin{document}HelloWorld\\end{document}",
    );
    let without_commands =
        compile_main(b"\\documentclass{article}\\begin{document}HelloWorld\\end{document}");
    assert_eq!(with_commands.status, CompileStatus::Ok);
...
#[test]
fn preamble_fancyhf_missing_group_is_not_implemented() {
    let result =
        compile_main(b"\\documentclass{article}\\fancyhf X\\begin{document}HelloWorld\\end{document}");
    assert_eq!(result.status, CompileStatus::NotImplemented);
    assert!(result.main_xdv_bytes.is_empty());
}

#[test]
fn preamble_fancypagestyle_with_begin_is_not_implemented() {
    let result = compile_main(
        b"\\documentclass{article}\\fancypagestyle{plain}{\\begin{document}}\\begin{document}HelloWorld\\end{document}",
    );
    assert_eq!(result.status, CompileStatus::NotImplemented);
    assert!(result.main_xdv_bytes.is_empty());
}
