...
#[test]
fn equation_label_emits_eq_marker_ok() {
    let baseline = baseline_char_count();
    let mut mount = Mount::default();
    let main =
        br"\documentclass{article}\begin{document}A\begin{equation}x\label{a}\end{equation}B\end{document}";
    assert!(mount.add_file(b"main.tex", main).is_ok());
    let result = compile_request_v0(&mut mount, &valid_request());
    assert_eq!(result.status, CompileStatus::Ok);
    ...
    assert_eq!(total, 819_200);
}

#[test]
fn theorem_label_emits_lbl_marker_ok() {
    let baseline = baseline_char_count();
    let mut mount = Mount::default();
    let main =
        br"\documentclass{article}\begin{document}A\begin{theorem}x\label{a}\end{theorem}B\end{document}";
    assert!(mount.add_file(b"main.tex", main).is_ok());
    let result = compile_request_v0(&mut mount, &valid_request());
    assert_eq!(result.status, CompileStatus::Ok);
    assert!(validate_dvi_v2_text_page_v0(&result.main_xdv_bytes));
...
#[test]
fn label_outside_env_still_noop_ok() {
    let baseline = baseline_char_count();
    let mut mount = Mount::default();
    let main = br"\documentclass{article}\begin{document}A\label{a}B\end{document}";
    assert!(mount.add_file(b"main.tex", main).is_ok());
    let result = compile_request_v0(&mut mount, &valid_request());
    assert_eq!(result.status, CompileStatus::Ok);
    assert!(validate_dvi_v2_text_page_v0(&result.main_xdv_bytes));
