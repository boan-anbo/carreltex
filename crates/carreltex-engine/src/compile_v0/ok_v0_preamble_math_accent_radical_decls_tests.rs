...
#[test]
fn declare_math_accent_preamble_is_ok_and_output_matches_without_it() {
    let with_decl = compile_main(
        b"\\documentclass{article}\\DeclareMathAccent{\\Foo}{\\mathord}{symbols}{\"0B}\\begin{document}HelloWorld\\end{document}",
    );
    let without_decl =
...
#[test]
fn declare_math_radical_preamble_is_ok_and_output_matches_without_it() {
    let with_decl = compile_main(
        b"\\documentclass{article}\\DeclareMathRadical{\\Foo}{symbols}{\"70}{largesymbols}{\"0A}\\begin{document}HelloWorld\\end{document}",
    );
    let without_decl =
        compile_main(b"\\documentclass{article}\\begin{document}HelloWorld\\end{document}");
    assert_eq!(with_decl.status, CompileStatus::Ok);
...
#[test]
fn declare_math_accent_bad_first_arg_shape_is_not_implemented() {
