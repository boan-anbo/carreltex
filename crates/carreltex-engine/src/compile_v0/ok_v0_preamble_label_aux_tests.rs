...
#[test]
fn preamble_label_aux_bundle_is_ok_and_output_matches_without_commands() {
    let with_commands = compile_main(
        b"\\documentclass{article}\\label{pre}\\ref{sec}\\pageref{pg}\\addtocontents{toc}{{alpha}}\\addcontentsline{toc}{section}{alpha}\\begin{document}HelloWorld\\end{document}",
    );
...
#[test]
fn preamble_label_aux_missing_group_is_not_implemented() {
    let result = compile_main(
        b"\\documentclass{article}\\addcontentsline{toc}{section}\\begin{document}HelloWorld\\end{document}",
    );
