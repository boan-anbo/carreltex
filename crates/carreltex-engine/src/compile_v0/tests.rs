...
use carreltex_core::{
    CompileRequestV0, CompileStatus, Mount, DEFAULT_COMPILE_MAIN_MAX_LOG_BYTES_V0, MAX_LOG_BYTES_V0,
};
...
#[test]
fn compile_request_rejects_invalid_ok_layout_controls() {
    let mut mount = Mount::default();
    assert!(mount.add_file(b"main.tex", valid_main()).is_ok());
    ...
    request = valid_request();
    request.ok_max_lines_per_page_v0 = Some(201);
    let result = compile_request_v0(&mut mount, &request);
    assert_eq!(result.status, CompileStatus::InvalidInput);
    assert!(result.log_bytes.ends_with(b"request_invalid"));

    request = valid_request();
    request.ok_line_advance_sp_v0 = Some(10_000_001);
    let result = compile_request_v0(&mut mount, &request);
    assert_eq!(result.status, CompileStatus::InvalidInput);
    assert!(result.log_bytes.ends_with(b"request_invalid"));

    request = valid_request();
    request.ok_glyph_advance_sp_v0 = Some(0);
    let result = compile_request_v0(&mut mount, &request);
    assert_eq!(result.status, CompileStatus::InvalidInput);
    assert!(result.log_bytes.ends_with(b"request_invalid"));
}
...
fn compile_request_log_is_truncated_by_max_log_bytes() {
    let mut mount = Mount::default();
    let unsupported_main = b"\\documentclass{article}\n\\begin{document}\n\\foo\n\\end{document}\n";
...
fn input_expands_tokens_from_subfile() {
    ...
    let main_without_input = b"\\documentclass{article}\n\\begin{document}\nHi\n\\end{document}\n";
    ...
    let char_count_with_input = stats_u64_field(&result_with_input.tex_stats_json, "char_count")
        .expect("char_count should exist");
...
fn csname_with_invalid_inner_tokens_is_invalid() {
    let mut mount = Mount::default();
    assert!(mount
        .add_file(b"main.tex", b"\\csname\\foo\\endcsname")
        .is_ok());
...
fn global_prefix_without_def_is_invalid() {
    ...
    assert!(result
        .log_bytes
        .ends_with(b"macro_global_prefix_unsupported"));
}
#[test]
fn stacked_global_prefix_without_def_is_invalid() {
    let mut mount = Mount::default();
    assert!(mount
        .add_file(b"main.tex", b"\\global\\global\\foo")
        .is_ok());
