...
#[test]
fn inline_control_math_emits_inline_marker_ok() {
...
#[test]
fn display_control_math_emits_block_marker_ok() {
    let baseline = baseline_char_count();
    let mut mount = Mount::default();
    let main = b"\\documentclass{article}\\begin{document}A\\[X\\]B\\end{document}";
    assert!(mount.add_file(b"main.tex", main).is_ok());
...
#[test]
fn unclosed_control_inline_math_falls_back_to_not_implemented() {
    let mut mount = Mount::default();
    let main = b"\\documentclass{article}\\begin{document}A\\(X\\end{document}";
    assert!(mount.add_file(b"main.tex", main).is_ok());
    let result = compile_request_v0(&mut mount, &valid_request());
