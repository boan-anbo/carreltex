...
#[test]
fn itemize_item_with_optional_label_is_ok() {
    let baseline = baseline_char_count();
    let mut mount = Mount::default();
    let main =
        b"\\documentclass{article}\\begin{document}\\begin{itemize}\\item[ZZ] A\\end{itemize}\\end{document}";
...
#[test]
fn itemize_item_with_unclosed_label_is_not_implemented() {
    let mut mount = Mount::default();
    let main = b"\\documentclass{article}\\begin{document}\\begin{itemize}\\item[ZZ A\\end{itemize}\\end{document}";
    assert!(mount.add_file(b"main.tex", main).is_ok());
    let result = compile_request_v0(&mut mount, &valid_request());
