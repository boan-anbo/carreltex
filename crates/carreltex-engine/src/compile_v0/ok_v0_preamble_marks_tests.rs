...
#[test]
fn preamble_marks_are_ok_and_output_matches_without_commands() {
    let with_marks = compile_main(
        b"\\documentclass{article}\\markboth{left}{right}\\markright{solo}\\begin{document}HelloWorld\\end{document}",
    );
    let without_marks =
        compile_main(b"\\documentclass{article}\\begin{document}HelloWorld\\end{document}");
    assert_eq!(with_marks.status, CompileStatus::Ok);
    assert_eq!(without_marks.status, CompileStatus::Ok);
    assert!(with_marks.log_bytes.is_empty());
    assert!(validate_dvi_v2_text_page_v0(&with_marks.main_xdv_bytes));
    assert_eq!(
        right3_positive_total(&with_marks),
        right3_positive_total(&without_marks)
    );
}

#[test]
fn preamble_marks_missing_group_is_not_implemented() {
    let result = compile_main(
        b"\\documentclass{article}\\markboth{left}\\begin{document}HelloWorld\\end{document}",
    );
