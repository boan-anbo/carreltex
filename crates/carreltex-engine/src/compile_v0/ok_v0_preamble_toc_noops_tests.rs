...
#[test]
fn preamble_toc_noops_are_ok_and_output_matches_without_commands() {
    let with_commands = compile_main(
        b"\\documentclass{article}\\tableofcontents\\listoffigures\\listoftables\\setuptoc{{alpha}}\\begin{document}HelloWorld\\end{document}",
    );
...
#[test]
fn preamble_setuptoc_missing_group_is_not_implemented() {
    let result = compile_main(
        b"\\documentclass{article}\\setuptoc\\begin{document}HelloWorld\\end{document}",
    );
