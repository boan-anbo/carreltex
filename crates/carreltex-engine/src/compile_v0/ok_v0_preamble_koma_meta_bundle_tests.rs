...
#[test]
fn preamble_koma_meta_bundle_is_ok_and_output_matches_without_commands() {
    let with_bundle = compile_main(
        b"\\documentclass{article}\\publishers{P}\\subject{S}\\titlehead{TH}\\subtitle{Sub}\\dedication{D}\\extratitle{ET}\\begin{document}HelloWorld\\end{document}",
    );
    let without_bundle =
...
#[test]
fn preamble_koma_meta_missing_group_is_not_implemented() {
    let result = compile_main(
        b"\\documentclass{article}\\subtitle\\begin{document}HelloWorld\\end{document}",
    );
