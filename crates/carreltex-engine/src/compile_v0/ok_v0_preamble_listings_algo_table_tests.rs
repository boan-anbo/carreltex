...
#[test]
fn preamble_listings_algo_table_bundle_is_ok_and_output_matches_without_decls() {
    let with_decls = compile_main(
        b"\\documentclass{article}\\lstset{basicstyle=\\ttfamily,breaklines=true}\\lstdefinestyle{mystyle}{numbers=left}\\lstnewenvironment{myenv}{\\small}{\\normalsize}\\SetKwInput{KwData}{Input}\\SetKw{KwTo}{to}\\newcolumntype{L}{>{\\raggedright\\arraybackslash}p{3cm}}\\begin{document}HelloWorld\\end{document}",
    );
    let without_decls =
...
#[test]
fn preamble_lstdefinestyle_missing_group_is_not_implemented() {
    let result = compile_main(
        b"\\documentclass{article}\\lstdefinestyle{mystyle}\\begin{document}HelloWorld\\end{document}",
    );
...
#[test]
fn preamble_lstset_begin_in_group_is_not_implemented() {
    let result = compile_main(
        b"\\documentclass{article}\\lstset{\\begin{document}}\\begin{document}HelloWorld\\end{document}",
    );
    assert_eq!(result.status, CompileStatus::NotImplemented);
    assert!(result.main_xdv_bytes.is_empty());
}
