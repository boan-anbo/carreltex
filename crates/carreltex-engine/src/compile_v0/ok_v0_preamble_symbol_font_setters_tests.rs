...
#[test]
fn set_symbol_font_preamble_is_ok_and_output_matches_without_it() {
    let with_decl = compile_main(
        b"\\documentclass{article}\\SetSymbolFont{operators}{normal}{OT1}{cmr}{m}{n}\\begin{document}HelloWorld\\end{document}",
    );
    let without_decl =
...
#[test]
fn declare_symbol_font_alphabet_preamble_is_ok_and_output_matches_without_it() {
    let with_decl = compile_main(
        b"\\documentclass{article}\\DeclareSymbolFontAlphabet{\\Foo}{operators}\\begin{document}HelloWorld\\end{document}",
    );
    let without_decl =
        compile_main(b"\\documentclass{article}\\begin{document}HelloWorld\\end{document}");
    assert_eq!(with_decl.status, CompileStatus::Ok);
...
#[test]
fn declare_symbol_font_alphabet_bad_first_arg_shape_is_not_implemented() {
