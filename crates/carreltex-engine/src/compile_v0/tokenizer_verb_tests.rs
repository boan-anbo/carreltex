...
fn empty_doc_baseline_char_count() -> u64 {
...
#[test]
fn verb_payload_is_tokenized_and_counted_in_stats() {
    let baseline = empty_doc_baseline_char_count();
    let mut mount = Mount::default();
    let main = b"\\documentclass{article}\n\\begin{document}\n\\verb|abc|\n\\end{document}\n";
    assert!(mount.add_file(b"main.tex", main).is_ok());
    let result = compile_request_v0(&mut mount, &valid_request());
...
#[test]
fn malformed_verb_maps_to_specific_tokenizer_reason() {
    let mut mount = Mount::default();
    let main = b"\\documentclass{article}\n\\begin{document}\n\\verb|abc\n\\end{document}\n";
    assert!(mount.add_file(b"main.tex", main).is_ok());
    let result = compile_request_v0(&mut mount, &valid_request());
    assert_eq!(result.status, CompileStatus::InvalidInput);
    assert!(result.log_bytes.ends_with(b"tokenizer_verb_not_supported"));
}
