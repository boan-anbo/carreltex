...
#[test]
fn preamble_legacy_baseline_bundle_is_ok_and_output_matches_without_decls() {
    let with_decls = compile_main(
        b"\\documentclass{article}\\usepackage{amsmath}\\usepackage[dvips]{graphicx}\\bibliographystyle{plain}\\bibliography{refs}\\theoremstyle{plain}\\newtheorem{thm}{Theorem}\\newtheorem*{lemma}{Lemma}\\newtheorem{lem}[thm]{Lemma}\\newtheorem{cor}{Corollary}[section]\\begin{document}HelloWorld\\end{document}",
    );
    let without_decls =
...
#[test]
fn preamble_legacy_baseline_missing_group_is_not_implemented() {
    let result =
        compile_main(b"\\documentclass{article}\\usepackage foo\\begin{document}HelloWorld\\end{document}");
