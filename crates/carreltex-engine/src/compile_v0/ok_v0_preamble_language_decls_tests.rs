...
#[test]
fn preamble_language_decls_are_ok_and_output_matches_without_decls() {
    let with_decls = compile_main(
        b"\\documentclass{article}\\selectlanguage{english}\\setmainlanguage[variant=american]{english}\\setdefaultlanguage{english}\\setotherlanguage[variant=latin]{french}\\begin{document}HelloWorld\\end{document}",
    );
    let without_decls =
...
#[test]
fn preamble_language_decl_missing_group_is_not_implemented() {
    let result =
        compile_main(b"\\documentclass{article}\\setmainlanguage\\begin{document}HelloWorld\\end{document}");
    assert_eq!(result.status, CompileStatus::NotImplemented);
    assert!(result.main_xdv_bytes.is_empty());
}

#[test]
fn preamble_language_decl_control_seq_in_group_is_not_implemented() {
    let result = compile_main(
        b"\\documentclass{article}\\setotherlanguage{\\foo}\\begin{document}HelloWorld\\end{document}",
    );
    assert_eq!(result.status, CompileStatus::NotImplemented);
    assert!(result.main_xdv_bytes.is_empty());
}
