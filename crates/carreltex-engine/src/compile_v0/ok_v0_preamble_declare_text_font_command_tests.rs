...
#[test]
fn declare_text_font_command_preamble_is_ok_and_output_matches_without_it() {
    let with_decl = compile_main(
        b"\\documentclass{article}\\DeclareTextFontCommand{\\Foo}{ABC}\\begin{document}HelloWorld\\end{document}",
    );
    let without_decl =
...
#[test]
fn declare_text_font_command_body_with_controlseq_is_not_implemented() {
    let result = compile_main(
        b"\\documentclass{article}\\DeclareTextFontCommand{\\Foo}{\\cite{X}}\\begin{document}HelloWorld\\end{document}",
    );
