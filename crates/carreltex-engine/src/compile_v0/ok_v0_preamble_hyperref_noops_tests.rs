...
#[test]
fn preamble_hyperref_noops_are_ok_and_output_matches_without_commands() {
    let with_commands = compile_main(
        b"\\documentclass{article}\\pdfstringdefDisableCommands{{abc}}\\AtBeginShipout{{xy}}\\AtBeginShipoutNext{{z}}\\begin{document}HelloWorld\\end{document}",
    );
...
#[test]
fn preamble_hyperref_missing_group_is_not_implemented() {
    let result = compile_main(
        b"\\documentclass{article}\\AtBeginShipout\\begin{document}HelloWorld\\end{document}",
    );
