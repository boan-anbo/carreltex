...
fn superscript_transparent_ok() {
    ...
    let main = b"\\documentclass{article}\\begin{document}A\\textsuperscript{B}C\\end{document}";
...
#[test]
fn subscript_missing_arg_not_implemented() {
    let mut mount = Mount::default();
    let main = b"\\documentclass{article}\\begin{document}\\textsubscript XYZ\\end{document}";
    assert!(mount.add_file(b"main.tex", main).is_ok());
    let result = compile_request_v0(&mut mount, &valid_request());
