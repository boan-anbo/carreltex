...
#[test]
fn declare_text_composite_command_preamble_is_ok_and_output_matches_without_it() {
    let with_decl = compile_main(
        b"\\documentclass{article}\\DeclareTextCompositeCommand{\\Foo}{T1}{32}{ABC}\\begin{document}HelloWorld\\end{document}",
    );
    let without_decl =
...
#[test]
fn declare_text_composite_command_body_with_controlseq_is_not_implemented() {
    let result = compile_main(
        b"\\documentclass{article}\\DeclareTextCompositeCommand{\\Foo}{T1}{32}{\\cite{X}}\\begin{document}HelloWorld\\end{document}",
    );
...
#[test]
fn declare_text_composite_command_bad_first_arg_shape_is_not_implemented() {
    let result = compile_main(
        b"\\documentclass{article}\\DeclareTextCompositeCommand{Foo}{T1}{32}{ABC}\\begin{document}HelloWorld\\end{document}",
    );
    assert_eq!(result.status, CompileStatus::NotImplemented);
