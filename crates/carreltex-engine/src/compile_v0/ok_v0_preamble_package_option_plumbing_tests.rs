...
#[test]
fn package_option_plumbing_preamble_commands_are_ok_and_output_matches_without_them() {
    let with_commands = compile_main(
        b"\\documentclass{article}\\RequirePackage{graphicx}\\RequirePackage[utf8]{inputenc}\\PassOptionsToPackage{dvips}{graphicx}\\PassOptionsToClass{oneside}{article}\\ExecuteOptions{final}\\begin{document}HelloWorld\\end{document}",
    );
...
#[test]
fn requirepackage_missing_group_is_not_implemented() {
    let result = compile_main(
        b"\\documentclass{article}\\RequirePackage[utf8]\\begin{document}HelloWorld\\end{document}",
    );
...
#[test]
fn executeoptions_group_with_control_seq_is_not_implemented() {
    let result = compile_main(
        b"\\documentclass{article}\\ExecuteOptions{\\foo}\\begin{document}HelloWorld\\end{document}",
    );
    assert_eq!(result.status, CompileStatus::NotImplemented);
    assert!(result.main_xdv_bytes.is_empty());
}
