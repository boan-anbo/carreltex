...
#[test]
fn enquote_emits_quoted_text_ok() {
    ...
    let main = b"\\documentclass{article}\\begin{document}A\\enquote{B}C\\end{document}";
    ...
    assert_eq!(total, 327_680);
}

#[test]
fn quote_missing_arg_not_implemented() {
    let mut mount = Mount::default();
    let main = b"\\documentclass{article}\\begin{document}\\quote XYZ\\end{document}";
    assert!(mount.add_file(b"main.tex", main).is_ok());
    let result = compile_request_v0(&mut mount, &valid_request());
