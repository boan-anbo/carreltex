...
#[test]
fn declare_math_alphabet_preamble_is_ok_and_output_matches_without_it() {
    let with_decl = compile_main(
        b"\\documentclass{article}\\DeclareMathAlphabet{\\Foo}{T1}{cmr}{m}{n}\\begin{document}HelloWorld\\end{document}",
    );
    let without_decl =
...
#[test]
fn set_math_alphabet_preamble_is_ok_and_output_matches_without_it() {
    let with_decl = compile_main(
        b"\\documentclass{article}\\SetMathAlphabet{\\Foo}{normal}{T1}{cmr}{m}{n}\\begin{document}HelloWorld\\end{document}",
    );
    let without_decl =
        compile_main(b"\\documentclass{article}\\begin{document}HelloWorld\\end{document}");
    assert_eq!(with_decl.status, CompileStatus::Ok);
...
#[test]
fn declare_math_alphabet_bad_first_arg_shape_is_not_implemented() {
