...
#[test]
fn declare_font_encoding_empty_groups_is_not_implemented() {
    let result = compile_main(
        b"\\documentclass{article}\\DeclareFontEncoding{T1}{}{}\\begin{document}HelloWorld\\end{document}",
    );
...
#[test]
fn declare_font_encoding_non_empty_preamble_is_ok_and_output_matches_without_it() {
    let with_decl = compile_main(
        b"\\documentclass{article}\\DeclareFontEncoding{T1}{A}{B}\\begin{document}HelloWorld\\end{document}",
    );
    let without_decl =
...
#[test]
fn declare_font_substitution_preamble_is_ok_and_output_matches_without_it() {
    let with_decl = compile_main(
        b"\\documentclass{article}\\DeclareFontSubstitution{T1}{cmr}{m}{n}\\begin{document}HelloWorld\\end{document}",
    );
    let without_decl =
        compile_main(b"\\documentclass{article}\\begin{document}HelloWorld\\end{document}");
    assert_eq!(with_decl.status, CompileStatus::Ok);
