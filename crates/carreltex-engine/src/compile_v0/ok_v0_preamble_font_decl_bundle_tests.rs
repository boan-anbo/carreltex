...
#[test]
fn font_decl_bundle_preamble_is_ok_and_output_matches_without_decls() {
    let with_decls = compile_main(
        b"\\documentclass{article}\\DeclareFontFamily{T1}{cmr}{opts}\\DeclareFontShape{T1}{cmr}{m}{n}{spec}{opts}\\DeclareFontEncodingDefaults{A}{B}\\DeclareFontSeriesDefault{T1}{cmr}{m}\\DeclareFontShapeDefault{T1}{cmr}{n}\\begin{document}HelloWorld\\end{document}",
    );
    let without_decls =
...
#[test]
fn font_decl_bundle_empty_group_is_not_implemented() {
    let result = compile_main(
        b"\\documentclass{article}\\DeclareFontFamily{T1}{}{opts}\\begin{document}HelloWorld\\end{document}",
    );
