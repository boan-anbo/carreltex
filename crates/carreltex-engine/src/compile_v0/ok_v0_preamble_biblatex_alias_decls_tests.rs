...
#[test]
fn biblatex_alias_decls_preamble_is_ok_and_output_matches_without_decls() {
    let with_decls = compile_main(
        b"\\documentclass{article}\\DeclareLanguageMapping{english}{english-apa}\\DeclareBibliographyAlias{book}{mvbook}\\DeclareNameAlias{default}{family-given}\\DeclareListAlias{default}{short}\\DeclareFieldAlias{title}{maintitle}\\begin{document}HelloWorld\\end{document}",
    );
    let without_decls =
...
#[test]
fn biblatex_alias_decl_missing_group_is_not_implemented() {
    let result = compile_main(
        b"\\documentclass{article}\\DeclareNameAlias{default}\\begin{document}HelloWorld\\end{document}",
    );
...
#[test]
fn biblatex_alias_decl_control_seq_inside_group_is_not_implemented() {
    let result = compile_main(
        b"\\documentclass{article}\\DeclareFieldAlias{title}{\\foo}\\begin{document}HelloWorld\\end{document}",
    );
    assert_eq!(result.status, CompileStatus::NotImplemented);
    assert!(result.main_xdv_bytes.is_empty());
}
