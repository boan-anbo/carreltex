...
#[test]
fn preamble_fancyhdr_glue_bundle_is_ok_and_output_matches_without_commands() {
    let with_commands = compile_main(
        b"\\documentclass{article}\\usepackage{fancyhdr}\\newcommand{\\headrulewidth}{0pt}\\newcommand{\\footrulewidth}{0pt}\\leftmark\\rightmark\\nouppercase{A}\\MakeUppercase{B}\\MakeLowercase{C}\\renewcommand{\\headrulewidth}{0.4pt}\\renewcommand{\\footrulewidth}{0pt}\\setlength{\\headheight}{14pt}\\setlength{\\footskip}{24pt}\\begin{document}HelloWorld\\end{document}",
    );
    let without_commands =
        compile_main(b"\\documentclass{article}\\usepackage{fancyhdr}\\newcommand{\\headrulewidth}{0pt}\\newcommand{\\footrulewidth}{0pt}\\begin{document}HelloWorld\\end{document}");
...
#[test]
fn preamble_fancyhdr_glue_missing_group_is_not_implemented() {
    let result = compile_main(
        b"\\documentclass{article}\\nouppercase X\\begin{document}HelloWorld\\end{document}",
    );
