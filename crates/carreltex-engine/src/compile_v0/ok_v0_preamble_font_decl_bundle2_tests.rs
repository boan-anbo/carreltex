...
#[test]
fn font_decl_bundle2_preamble_is_ok_and_output_matches_without_decls() {
    let with_decls = compile_main(
        b"\\documentclass{article}\\DeclareFontEncoding{T1}{A}{B}\\DeclareFontSubstitution{T1}{cmr}{m}{n}\\DeclareFontSeriesDefault{T1}{cmr}{m}\\DeclareFontShapeDefault{T1}{cmr}{n}\\DeclareFontFamilyDefault{T1}{cmr}\\begin{document}HelloWorld\\end{document}",
    );
    let without_decls =
...
#[test]
fn font_decl_bundle2_empty_group_is_not_implemented() {
    let result = compile_main(
        b"\\documentclass{article}\\DeclareFontFamilyDefault{T1}{}\\begin{document}HelloWorld\\end{document}",
    );
