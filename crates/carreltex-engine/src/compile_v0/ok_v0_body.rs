...
use super::ok_v0_env_refs::{emit_ok_markers_in_env_v0, OkEnvMarkersV0};
use super::ok_v0_optional_brackets::{
...
use super::ok_v0_dollar_math::{consume_display_math_dollar_span_v0, consume_inline_math_dollar_span_v0};
use super::ok_v0_ensuremath::{consume_inline_math_group_span_v0, consume_math_control_span_v0};
use super::ok_v0_lists::{
    begin_list_v0, emit_list_item_v0, end_list_v0, list_stack_active_v0, ListStateV0,
};
use super::ok_v0_biblabel::consume_optional_bibitem_label_fragment_v0;
use super::ok_v0_markers::{is_cite_marker_command_v0, is_ref_marker_command_v0, ok_marker_command_v0};
use super::ok_v0_env_support::{
...
pub(super) fn skip_spaces_until(tokens: &[TokenV0], mut index: usize, end_limit: usize) -> usize {
...
pub(super) fn consume_group_literal(tokens: &[TokenV0], mut index: usize, literal: &[u8]) -> Option<usize> {
...
pub(super) fn consume_char_space_group_non_empty(tokens: &[TokenV0], mut index: usize) -> Option<usize> {
    if !matches!(tokens.get(index), Some(TokenV0::BeginGroup)) {
        return None;
    }
    index += 1;
...
pub(super) fn consume_bracket_options_non_empty(tokens: &[TokenV0], mut index: usize) -> Option<usize> {
...
pub(super) fn consume_char_space_nested_group_non_empty_v0(
    ...
    let (_, inner_end, next_index) =
        consume_balanced_group_bounds_v0(tokens, cursor, super::MAX_OK_GROUP_DEPTH_V0, end)?;
...
pub(super) fn consume_ok_body_token_v0(
    tokens: &[TokenV0],
    index: usize,
    end: usize,
    allow_nested_groups: bool,
    ...
    match tokens.get(index) {
        ...
        Some(TokenV0::ControlSeq(name)) if name.as_slice() == b"(" => {
            let next_index =
                consume_math_control_span_v0(tokens, index, end, b")", super::MAX_OK_MATH_SCAN_TOKENS_V0)?;
            ...
            if matches!(name.as_slice(), b"ensuremath" | b"text" | b"textnormal" | b"mathrm" | b"mathit" | b"mathbf" | b"mathbb" | b"mathcal" | b"mathsf" | b"mathtt") =>
        {
            let next_index = consume_inline_math_group_span_v0(
                ...
                super::MAX_OK_ENSUREMATH_TOKENS_V0,
        ...
        Some(TokenV0::Char(b'$')) if matches!(tokens.get(index + 1), Some(TokenV0::Char(b'$'))) => {
            let next_index =
                consume_display_math_dollar_span_v0(tokens, index, end, super::MAX_OK_DOLLAR_MATH_TOKENS_V0)?;
            emit_ok_display_math_marker_v0(body, previous_was_space);
            Some(next_index)
        }
        Some(TokenV0::Char(b'$')) => consume_inline_math_dollar_span_v0(
            ...
            super::MAX_OK_DOLLAR_MATH_TOKENS_V0,
        )
        .map(|next_index| {
            emit_ok_inline_math_marker_v0(body, previous_was_space);
            next_index
        ...
        Some(TokenV0::ControlSeq(name))
            if super::ok_v0_noops::is_ok_noop_command_v0(name.as_slice()) =>
        {
            super::ok_v0_noops::consume_ok_noop_command_v0(tokens, index, end, name.as_slice())
        }
        ...
        Some(TokenV0::ControlSeq(name)) if name.as_slice() == b"includegraphics" => {
            let mut cursor =
                consume_optional_simple_bracket_span_v0(tokens, index + 1, end, super::MAX_OK_BRACKET_BYTES_V0)?;
        ...
        Some(TokenV0::ControlSeq(name)) if name.as_slice() == b"caption" => {
            ...
            cursor = consume_optional_simple_bracket_span_v0(tokens, cursor, end, super::MAX_OK_BRACKET_BYTES_V0)?;
            ...
            if ok_marker_command_v0(name.as_slice()).is_some() =>
        {
            ...
            if is_cite_marker_command_v0(name.as_slice()) || is_ref_marker_command_v0(name.as_slice()) {
                let max_note_tokens = if is_cite_marker_command_v0(name.as_slice()) {
                    super::MAX_OK_CITE_NOTE_TOKENS_V0
                } else {
                    super::MAX_OK_REF_NOTE_TOKENS_V0
                };
            ...
            if is_supported_ok_heading_command_v0(name.as_slice()) =>
        {
            ...
            cursor = consume_optional_heading_short_title_v0(
                ...
                super::MAX_OK_HEADING_SHORT_TOKENS_V0,
                super::MAX_OK_GROUP_DEPTH_V0,
            )?;
            ...
            if !allow_nested_groups && name.as_slice() == b"begin" =>
        {
                ...
                consume_named_environment_span_v0(tokens, index, end)
            {
                ...
                if is_supported_display_math_env_v0(&env_name) {
                    if next_index - index > super::MAX_OK_MATH_ENV_TOKENS_V0 {
                        return None;
                    }
                    emit_ok_display_math_marker_v0(body, previous_was_space);
                    emit_ok_markers_in_env_v0(
            ...
            match list_env {
                Some(ListStateV0::Thebibliography) => {
                    let (after_optional, label) = consume_optional_bibitem_label_fragment_v0(
                        ...
                        super::MAX_OK_BIBLABEL_TOKENS_V0,
                        super::MAX_OK_GROUP_DEPTH_V0,
                    )?;
                    let next_index =
        ...
        Some(TokenV0::BeginGroup) if allow_nested_groups => {
            let (inner_start, inner_end, next_index) =
                consume_balanced_group_bounds_v0(tokens, index, super::MAX_OK_GROUP_DEPTH_V0, end)?;
            consume_ok_body_range_v0(
                tokens,
                inner_start,
                inner_end,
