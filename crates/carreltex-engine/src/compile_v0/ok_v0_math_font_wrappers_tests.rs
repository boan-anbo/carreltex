...
fn assert_math_font_wrapper_ok(control_word: &str) {
    let mut mount = Mount::default();
    let main = format!(
        "\\documentclass{{article}}\\begin{{document}}A\\{control_word}{{x}}B\\end{{document}}"
    );
...
#[test]
fn math_font_wrappers_emit_inline_math_marker_ok() {
    for control_word in ["mathbf", "mathbb", "mathcal", "mathsf", "mathtt"] {
        assert_math_font_wrapper_ok(control_word);
    }
}

#[test]
fn math_font_wrapper_missing_group_is_not_implemented() {
    let mut mount = Mount::default();
    let main = b"\\documentclass{article}\\begin{document}A\\mathbf xB\\end{document}";
