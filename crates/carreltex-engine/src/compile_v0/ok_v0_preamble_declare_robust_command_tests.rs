...
#[test]
fn declare_robust_command_zero_arity_preamble_is_ok_and_output_matches_without_it() {
    let with_decl = compile_main(
        b"\\documentclass{article}\\DeclareRobustCommand{\\Foo}{ABC}\\begin{document}HelloWorld\\end{document}",
    );
    let without_decl =
...
#[test]
fn declare_robust_command_one_arity_preamble_is_ok_and_output_matches_without_it() {
    let with_decl = compile_main(
        b"\\documentclass{article}\\DeclareRobustCommand{\\Foo}[1]{ABC}\\begin{document}HelloWorld\\end{document}",
    );
    let without_decl =
        compile_main(b"\\documentclass{article}\\begin{document}HelloWorld\\end{document}");
    assert_eq!(with_decl.status, CompileStatus::Ok);
...
#[test]
fn declare_robust_command_body_with_controlseq_is_not_implemented() {
    let result = compile_main(
        b"\\documentclass{article}\\DeclareRobustCommand{\\Foo}{\\cite{X}}\\begin{document}HelloWorld\\end{document}",
    );
