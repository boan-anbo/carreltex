...
#[test]
fn linebreak_control_symbol_emits_newline_marker_ok() {
    let baseline = baseline_char_count();
    let mut mount = Mount::default();
    let main = br"\documentclass{article}\begin{document}A\\B\end{document}";
    assert!(mount.add_file(b"main.tex", main).is_ok());
    let result = compile_request_v0(&mut mount, &valid_request());
    assert_eq!(result.status, CompileStatus::Ok);
...
#[test]
fn linebreak_control_symbol_star_form_is_supported() {
    let baseline = baseline_char_count();
    let mut mount = Mount::default();
    let main = br"\documentclass{article}\begin{document}A\\*B\end{document}";
    assert!(mount.add_file(b"main.tex", main).is_ok());
...
#[test]
fn linebreak_control_symbol_in_preamble_is_not_implemented() {
    let mut mount = Mount::default();
    let main = br"\documentclass{article}\\\begin{document}AB\end{document}";
    assert!(mount.add_file(b"main.tex", main).is_ok());
    let result = compile_request_v0(&mut mount, &valid_request());
