...
#[test]
fn declare_text_symbol_preamble_is_ok_and_output_matches_without_it() {
    let with_decl = compile_main(
        b"\\documentclass{article}\\DeclareTextSymbol{\\Foo}{T1}{32}\\begin{document}HelloWorld\\end{document}",
    );
    let without_decl =
...
#[test]
fn declare_text_accent_preamble_is_ok_and_output_matches_without_it() {
    let with_decl = compile_main(
        b"\\documentclass{article}\\DeclareTextAccent{\\Foo}{T1}{32}\\begin{document}HelloWorld\\end{document}",
    );
    let without_decl =
        compile_main(b"\\documentclass{article}\\begin{document}HelloWorld\\end{document}");
    assert_eq!(with_decl.status, CompileStatus::Ok);
...
#[test]
fn declare_text_symbol_bad_first_arg_shape_is_not_implemented() {
