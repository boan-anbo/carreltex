...
#[test]
fn tabular_env_stub_ok() {
    let baseline = baseline_char_count();
    let mut mount = Mount::default();
    let main = br"\documentclass{article}\begin{document}A\begin{tabular}{c}x\end{tabular}B\end{document}";
    assert!(mount.add_file(b"main.tex", main).is_ok());
    let result = compile_request_v0(&mut mount, &valid_request());
    assert_eq!(result.status, CompileStatus::Ok);
...
#[test]
fn longtable_env_stub_ok() {
    let baseline = baseline_char_count();
    let mut mount = Mount::default();
    let main = br"\documentclass{article}\begin{document}A\begin{longtable}\end{longtable}B\end{document}";
    assert!(mount.add_file(b"main.tex", main).is_ok());
    let result = compile_request_v0(&mut mount, &valid_request());
    assert_eq!(result.status, CompileStatus::Ok);
    assert!(validate_dvi_v2_text_page_v0(&result.main_xdv_bytes));
...
fn missing_end_is_not_implemented() {
    let mut mount = Mount::default();
    let main = br"\documentclass{article}\begin{document}\begin{tabular}X\end{document}";
    assert!(mount.add_file(b"main.tex", main).is_ok());
    let result = compile_request_v0(&mut mount, &valid_request());
