...
#[test]
fn biblatex_resource_preamble_commands_are_ok_and_output_matches_without_them() {
    let with_commands = compile_main(
        b"\\documentclass{article}\\addbibresource{refs.bib}\\addbibresource[location=remote]{more.bib}\\ExecuteBibliographyOptions{sorting=nyt}\\DeclareBibliographyCategory{primary}\\addtocategory{primary}{k1,k2}\\begin{document}HelloWorld\\end{document}",
    );
...
#[test]
fn addbibresource_missing_group_is_not_implemented() {
    let result = compile_main(
        b"\\documentclass{article}\\addbibresource[location=remote]\\begin{document}HelloWorld\\end{document}",
    );
...
#[test]
fn addtocategory_group_with_control_seq_is_not_implemented() {
    let result = compile_main(
        b"\\documentclass{article}\\addtocategory{primary}{\\foo}\\begin{document}HelloWorld\\end{document}",
    );
    assert_eq!(result.status, CompileStatus::NotImplemented);
    assert!(result.main_xdv_bytes.is_empty());
}
