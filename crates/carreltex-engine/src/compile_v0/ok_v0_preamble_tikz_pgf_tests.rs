...
#[test]
fn preamble_tikz_pgf_bundle_is_ok_and_output_matches_without_decls() {
    let with_decls = compile_main(
        b"\\documentclass{article}\\usetikzlibrary{arrows,positioning}\\usepgfplotslibrary{groupplots}\\tikzset{my/.style={draw,thick}}\\pgfplotsset{compat=1.18}\\pgfkeys{/my/path/.cd,key=value}\\pgfkeysalso{/my/other/.style={red}}\\begin{document}HelloWorld\\end{document}",
    );
    let without_decls =
...
#[test]
fn preamble_tikzset_missing_group_is_not_implemented() {
    let result =
        compile_main(b"\\documentclass{article}\\tikzset X\\begin{document}HelloWorld\\end{document}");
    assert_eq!(result.status, CompileStatus::NotImplemented);
    assert!(result.main_xdv_bytes.is_empty());
}

#[test]
fn preamble_tikzset_begin_in_group_is_not_implemented() {
    let result = compile_main(
        b"\\documentclass{article}\\tikzset{\\begin{document}}\\begin{document}HelloWorld\\end{document}",
    );
    assert_eq!(result.status, CompileStatus::NotImplemented);
    assert!(result.main_xdv_bytes.is_empty());
}
